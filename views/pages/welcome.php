<link rel="stylesheet" href="/views/css/welcome_style.css" />
<!--Section 1 : Lettre ouverte de la fondatrice-->
<section class="container_section">
    <section id="section_1">
        <article id="presentation_fondatrice">
            <img src="/views/asset/IMG/Belen_fondatrice.jpeg" alt="Photographie de présentation de la créatrice - Belén LEGER">
                <div class="contenu">
                    <h1>Derriére GENESIS : Le mot de Belén</h1>
                    <p><em>Chére Base Arrière,</em></p>
                    <p>Partager la vie d'un militaire est un engagement invisible, mais total. Partager celle d'un légionnaire, c'est accepter d'entrer dans un monde aux codes uniques, fait d'une immense fierté, mais aussi de très longues attentes et de silences imposés.</p>
                </div>
            <a href="#">Je veux découvrir la suite de la lettre ouverte !</a>
        </article>
    <!-- Les valeurs
        <div class="contenu_valeur">
            <div>
                    <img src="/views/asset/IMG/liberty.ico" alt="Première valeur : Liberté" class="valeur">
            </div>
            <div>
                    <img src="/views/asset/IMG/audacity.ico" alt="Deuxiéme valeur : l'Audace" class="valeur">
            </div>
            <div>
                    <img src="/views/asset/IMG/loyauty.ico" alt="Troisiéme valeur : la Loyauté" class="valeur">
            </div>
        </div> -->
    </section>
    <!--Section 2: new article => new update du site ou application-->
    <section class="section_2">
        <!-- <article>
            <h2>Les derniers articles publiés par nos membres</h2>
            <p>Ici, tu retrouveras les dernières actualités sur la sphére militaire</p>
        </article> -->
    <!--New article : Derniers articles publiés pour les utilisateurs-->
        <div>
            <div class="last_art">
                <a href="#">
                    <img src="/views/asset/IMG/best_coffee.jpg" alt="Illustration d'une tasse de café recevant du lait" class="art_img">
                </a>
            </div>

            <div class="last_art">
                <a href="#">
                    <img src="/views/asset/IMG/drink_with_friend.jpg" alt="Un verre à martini" class="art_img">
                    
                </a>
            </div>

            <div class="last_art">
                <a href="#">
                    <img src="/views/asset/IMG/boat_on_wall.jpg" alt="Un bateau sur un mur" class="art_img">
                    
                </a>
            </div>
        </div>
    </section>
</section>